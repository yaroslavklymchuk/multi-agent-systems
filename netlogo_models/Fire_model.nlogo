...
to setup
  ...
  set total-lakes-square count patches with [pcolor = blue]
  set burned-trees 0
  reset-ticks
end

to go
  if not any? fire-patches
    [ stop ]
  ask fire-patches
    [ ask neighbors with [pcolor = green] [
    let probability_spread spread_probability
    set fire_direction towards myself

    if (pcolor = green) [
      if (fire_direction = wind_direction) [
        set probability_spread probability_spread + wind_speed
      ]
    if (fire_direction != wind_direction) [
        set probability_spread probability_spread - wind_speed
      ]
    if (random-float 100) < probability_spread [ set pcolor red]
      ]
    ]
  set pcolor black
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot 100 * burned-trees / initial-trees"
...
<experiments>
  ...
  <experiment name="Density Experiment" repetitions="100" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="density" first="10" step="10" last="100"/>
    <steppedValueSet variable="lake-square" first="0" step="50" last="500"/>
    <enumeratedValueSet variable="spread_probability">
      <value value="90"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="lakes-count">
      <value value="10"/>
    </enumeratedValueSet>
    <steppedValueSet variable="wind_direction" first="0" step="45" last="315"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
